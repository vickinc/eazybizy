/**
 * Dedicated service for handling Ethereum blockchain transactions
...
export class EthereumTransactionService {
  ...
  static async getTransactions(
    ...
    if (!EtherscanAPIService.isConfigured('ethereum')) {
      throw new Error('Etherscan API not configured');
    }
  ...
  /**
   * Convert Ethereum blockchain transaction to standard transaction format
   */
  static async convertToStandardTransaction(
    ...
    const uniqueId = `ethereum-${blockchainTx.hash}-${blockchainTx.currency}-${blockchainTx.type}-${walletInfo.id}-${Date.now()}`;
    ...
    return {
      ...
      linkedEntry: {
        ...
        currency: blockchainTx.currency,
        description: `Live Ethereum transaction`,
        blockchain: 'ethereum'
      }
    };
  }
  ...
  /**
   * Check if Ethereum API is properly configured
   */
  static isConfigured(): boolean {
    return EtherscanAPIService.isConfigured('ethereum');
  }

  /**
   * Get supported currencies for Ethereum blockchain
   */
  ...
  /**
   * Validate if a currency is supported on Ethereum
   */
