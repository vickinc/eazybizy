/**
 * Dedicated service for handling Tron blockchain transactions
...
export class TronTransactionService {
  /**
   * Fetch Tron transactions for a wallet address
   */
  static async getTransactions(
    ...
    if (!TronGridService.isConfigured()) {
      throw new Error('TronGrid API not configured');
    }
    ...
    try {
      ...
      if (currency?.toUpperCase() === 'TRX') {
        ...
        transactions = await TronGridService.getTransactionHistory(address, {
          startDate,
          endDate,
          limit
        });
      } else if (currency) {
        // Fetch specific TRC-20 token transactions
        ...
        // Fetch all transactions (TRX + TRC-20)
        transactions = await TronGridService.getTransactionHistory(address, {
          startDate,
          endDate,
          limit
  ...
  /**
   * Convert Tron blockchain transaction to standard transaction format
   */
  static async convertToStandardTransaction(
    ...
    const uniqueId = `tron-${blockchainTx.hash}-${blockchainTx.currency}-${blockchainTx.type}-${walletInfo.id}-${Date.now()}`;
    ...
    return {
      ...
      description: `Tron ${blockchainTx.tokenType?.toUpperCase() || 'TRX'} ${isIncoming ? 'received from' : 'sent to'} ${isIncoming ? this.formatAddress(blockchainTx.from) : this.formatAddress(blockchainTx.to)}`,
      ...
      linkedEntry: {
        ...
        currency: blockchainTx.currency,
        description: `Live Tron transaction`,
        blockchain: 'tron'
      }
    };
  }
  ...
  /**
   * Check if Tron API is properly configured
   */
  static isConfigured(): boolean {
    return TronGridService.isConfigured();
  }

  /**
   * Get supported currencies for Tron blockchain
   */
  ...
  /**
   * Validate if a currency is supported on Tron
   */
