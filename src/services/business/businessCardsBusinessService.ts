...
export class BusinessCardsBusinessService {
  ...
  static downloadBusinessCardImage(card: BusinessCard): Promise<void> {
    return new Promise((resolve, reject) => {
      // Create a new element specifically for download instead of cloning
      ...
      downloadElement.innerHTML = `
        <div style="display: flex; align-items: flex-start; gap: 12px; margin-bottom: 16px;">
          <div style="width: 40px; height: 40px; background: white; border-radius: 4px; display: flex; align-items: center; justify-content: center; flex-shrink: 0;">
            ${card.company.logo && (card.company.logo.startsWith('data:') || card.company.logo.includes('http')) 
              ? `<img src="${card.company.logo}" alt="Logo" style="width: 32px; height: 32px; border-radius: 4px; object-fit: cover;">`
              : `<span style="color: #2563eb; font-weight: bold; font-size: 12px;">${card.company.logo || card.company.tradingName.substring(0, 2).toUpperCase()}</span>`
            }
          </div>
          <div style="flex: 1; min-width: 0;">
            <h3 style="margin: 0; font-size: 16px; font-weight: bold; color: ${templateStyles.textColor}; line-height: 1.2;">${card.company.tradingName}</h3>
            <p style="margin: 2px 0 0 0; font-size: 12px; opacity: 0.75; color: ${templateStyles.textColor}; line-height: 1.2;">${card.company.industry}</p>
          </div>
        </div>
        
        ${card.personName ? `
          <div style="margin-bottom: 16px;">
            <p style="margin: 0; font-size: 18px; font-weight: bold; color: ${templateStyles.textColor}; line-height: 1.2;">${card.personName}</p>
            ${card.position ? `<p style="margin: 2px 0 0 0; font-size: 14px; opacity: 0.85; color: ${templateStyles.textColor}; line-height: 1.2;">${card.position}</p>` : ''}
          </div>
        ` : ''}
        
        <div style="position: absolute; bottom: 24px; left: 24px; right: 88px;">
          <div style="display: flex; flex-direction: column; gap: 4px;">
            <div style="display: flex; align-items: center; gap: 8px; font-size: 12px; color: ${templateStyles.textColor};">
        ...
        <div style="position: absolute; bottom: 24px; right: 24px; width: 64px; height: 64px; background: white; border-radius: 4px; display: flex; align-items: center; justify-content: center;">
          <img src="${BusinessCardsBusinessService.generateQRCodeUrl(card.qrValue)}" alt="QR Code" style="width: 56px; height: 56px;">
        </div>
      `;
