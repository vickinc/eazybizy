...
export class GoogleCalendarService {
  ...
  async listEvents(calendarId: string = 'primary', timeMin?: Date, timeMax?: Date): Promise<calendar_v3.Schema$Event[]> {
    await this.initializeAuth();
  ...
  private convertToGoogleEvent(event: CalendarEvent): GoogleCalendarEventData {
    ...
    return {
      ...
      attendees: event.participants ? 
        JSON.parse(event.participants).map((email: string) => ({ email })) : undefined
    };
  }

  // Convert Google Calendar event to local format
  private convertFromGoogleEvent(googleEvent: calendar_v3.Schema$Event): Partial<CalendarEvent> {
