...
import { TaxTreatment, TaxApplicability } from '@/types/taxTreatment.types'
...
export interface TaxTreatmentsListParams {
  skip?: number
  take?: number
  searchTerm?: string
  category?: string
...
export interface TaxTreatmentsListResponse {
  data: TaxTreatment[]
  pagination: {
    total: number
    skip: number
...
export interface TaxTreatmentsStatistics {
  total: number
  totalActive: number
...
/**
 * Direct database service for Tax Treatments SSR
 ...
 */
export class TaxTreatmentsSSRService {
  /**
   * Get tax treatments with minimal fields for list view
   * Used during SSR for fast initial page load
   */
  static async getTaxTreatmentsForSSR(params: TaxTreatmentsListParams = {}): Promise<TaxTreatmentsListResponse> {
    const startTime = Date.now()
    
    // Default parameters optimized for SSR (minimal initial load for ultra-fast paint)
    const {
      skip = 0,
      take = 50, // Reasonable default for tax treatments
      searchTerm = '',
      category,
      isActive,
      applicability,
    ...
    const cacheKey = `tax-treatments:${skip}:${take}:${searchTerm}:${category}:${isActive}:${applicability}:${companyId}:${minRate}:${maxRate}:${sortField}:${sortDirection}`
    
    // Check cache first (30 second TTL for SSR)
    const cachedResult = ssrCache.get<TaxTreatmentsListResponse>(cacheKey)
    if (cachedResult) {
      return {
        ...cachedResult,
        cached: true,
    ...
    try {
      ...
      const [treatments, totalCount] = await Promise.all([
        prisma.taxTreatment.findMany({
          ...
          skip,
          take,
          // Select essential fields for tax treatment list display
          select: {
            id: true,
            code: true,
            name: true,
      ...
      // Transform to TaxTreatment format (matching the type interface)
      const transformedTreatments: TaxTreatment[] = treatments.map(treatment => ({
        id: treatment.id,
        code: treatment.code,
        name: treatment.name,
      ...
      const result: TaxTreatmentsListResponse = {
      ...
      console.error('TaxTreatmentsSSRService error:', error)
      throw new Error('Failed to fetch tax treatments for SSR')
    }
  }

  /**
   * Get tax treatments statistics separately (for async loading)
   * This expensive operation is separated from the main list query
   */
  static async getTaxTreatmentsStatistics(): Promise<TaxTreatmentsStatistics> {
    try {
      const [totalActive, totalInactive, allTreatments] = await Promise.all([
        prisma.taxTreatment.count({ where: { isActive: true } }),
        prisma.taxTreatment.count({ where: { isActive: false } }),
        prisma.taxTreatment.findMany({
          select: {
            category: true,
            applicability: true,
      ...
      console.error('TaxTreatmentsSSRService statistics error:', error)
      throw new Error('Failed to fetch tax treatments statistics')
    }
  }

  /**
   * Simple validation that user has access to tax treatments
  ...
  static async validateUserAccess(userId: string): Promise<boolean> {
    try {
      ...
      console.error('TaxTreatmentsSSRService auth error:', error)
      return false
    }
  }
}