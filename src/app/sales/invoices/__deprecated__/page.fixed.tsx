...
export default function InvoicesPageFixed() {
  ...
  const hasCompanyData = companies && companies.length > 0;
  const selectedCompanyId = globalSelectedCompany?.id || companies?.[0]?.id;
  ...
  if (!hasCompanyData) {
    return <PageLoading message="Loading company data..." />;
  }

  // Show loading state while invoice data loads
  if (!isLoaded) {
    return (
      ...
      <div className="min-h-screen bg-lime-50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          <InvoiceActions
            ...
            filteredInvoicesCount={filteredInvoices?.length || 0}
