...
import { 
  useVATTreatments, 
  useVATTreatmentsOperations,
  useBulkCreateVATTreatments
} from '@/hooks/useVATTreatmentsAPI';
...
export const useVATTreatmentManagementAPI = (): VATTreatmentManagementHook => {
  // Form State
  ...
  // API Hooks
  const { data: treatmentsResponse, isLoading, error } = useVATTreatments(queryParams);
  const { createTreatment, updateTreatment, deleteTreatment } = useVATTreatmentsOperations();
  const bulkCreate = useBulkCreateVATTreatments();
  ...
  const filteredTreatments = useMemo(() => {
    ...
    return [...treatments].sort((a, b) => {
      const aValue = a[sortConfig.field as keyof VATTreatment];
      const bValue = b[sortConfig.field as keyof VATTreatment];
  ...
  const stats = useMemo((): VATTreatmentStats => {
    if (treatmentsResponse?.statistics) {
      ...
      const byCategory: { [key in VATCategory]: number } = {
        standard: 0,
        reduced: 0,
        exempt: 0,
        special: 0,
  ...
  const handleDelete = useCallback(async (id: string) => {
    if (confirm('Are you sure you want to delete this VAT treatment? This action cannot be undone.')) {
      try {
  ...
  const handleFormSubmit = useCallback(async (e: React.FormEvent) => {
    ...
    // Validate form
    const errors = VATTreatmentValidationService.validateForm(formData);
  ...
  const exportToCSV = useCallback(() => {
    return VATTreatmentBusinessService.exportToCSV(filteredTreatments);
  }, [filteredTreatments]);

  const initializeDefaults = useCallback(async () => {
    try {
      const defaultTreatments = VATTreatmentBusinessService.getDefaultTreatments();
