...
export const useCalendarManagementDB = (
  ...
  const deletedAnniversaryIds = new Set<string>();
  ...
  databaseEvents.forEach(event => {
    ...
    if (descriptionToCheck) {
      ...
      // Check for deletion markers in description
      const deleteMatch = descriptionToCheck.match(/\[DELETED:([^\]]+)\]/);
      if (deleteMatch) {
        deletedAnniversaryIds.add(deleteMatch[1]);
      }
    }
  });
  ...
  const handleDeleteEvent = useCallback(async (eventId: string) => {
    try {
      ...
      if (isAutoGenerated) {
        // For auto-generated events, create a deletion marker in the database
        const deletionMarkerData: CalendarEventCreateRequest = {
          title: `DELETED_ANNIVERSARY_${eventId}`,
          description: `[DELETED:${eventId}]`,
        ...
        await createEventMutation.mutateAsync(deletionMarkerData);
      } else {
  ...
  }, [deleteEventMutation, createEventMutation]);
