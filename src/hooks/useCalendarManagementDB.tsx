...
export const useCalendarManagementDB = (
  ...
  // Track modified anniversary events (in-memory updates)
  const [modifiedAnniversaryEvents, setModifiedAnniversaryEvents] = useState<Map<string, CalendarEvent>>(new Map());
  ...
  const anniversaryEvents = CompanyAnniversaryService.generateAnniversaryEventsForCompanies(
    ...
    startDate, 
    endDate
  ).map(anniversaryEvent => {
    const calendarEvent = CompanyAnniversaryService.convertToCalendarEvent(anniversaryEvent);
    // Use modified version if it exists
    return modifiedAnniversaryEvents.get(calendarEvent.id) || calendarEvent;
  }).filter(event => {
    // Filter out deleted anniversary events
    const modified = modifiedAnniversaryEvents.get(event.id);
    return !modified || !(modified as any).__deleted;
  });

  // Combine database events with anniversary events (filtering already done above)
  ...
  // Helper function to clean description for editing (no longer needed but keeping for consistency)
  ...
  const handleUpdateEvent = useCallback(async () => {
    ...
    try {
      ...
      if (isAutoGenerated) {
        // For auto-generated events, update in-memory only
        const updatedEvent: CalendarEvent = {
          ...
          syncEnabled: formData.syncEnabled,
          targetCalendarId: formData.targetCalendarId,
          // Keep original anniversary properties
          createdAt: editingEvent.createdAt,
          updatedAt: new Date()
        };
        
        // Store the modified anniversary event in state
        setModifiedAnniversaryEvents(prev => {
          const newMap = new Map(prev);
          newMap.set(editingEvent.id, updatedEvent);
          return newMap;
        });
      } else {
  ...
  const handleDeleteEvent = useCallback(async (eventId: string) => {
    try {
      ...
      if (isAutoGenerated) {
        // For auto-generated events, mark as deleted in-memory
        setModifiedAnniversaryEvents(prev => {
          const newMap = new Map(prev);
          // Mark as deleted by setting a special deleted property
          const deletedEvent = { ...prev.get(eventId), __deleted: true } as CalendarEvent & { __deleted: boolean };
          newMap.set(eventId, deletedEvent);
          return newMap;
        });
      } else {
